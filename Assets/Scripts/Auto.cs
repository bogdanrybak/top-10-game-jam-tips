...
public static class Auto
{
	...
	public static IEnumerator MoveTo(this RectTransform transform, Vector3 target, float duration, Easer ease)
	{
		float elapsed = 0;
		var start = transform.anchoredPosition3D;
		var range = target - start;
		while (elapsed < duration)
		{
			elapsed = Mathf.MoveTowards(elapsed, duration, Time.deltaTime);
			transform.anchoredPosition3D = start + range * ease(elapsed / duration);
			yield return null;
		}
		transform.anchoredPosition3D = target;
	}

	public static IEnumerator MoveTo(this RectTransform transform, Vector3 target, float duration)
	{
		return MoveTo(transform, target, duration, Ease.Linear);
	}
	public static IEnumerator MoveTo(this RectTransform transform, Vector3 target, float duration, EaseType ease)
	{
	...
	public static IEnumerator MoveTo(this Transform transform, Vector3 target, float duration, Easer ease, System.Func<float> getTime)
    {
        float elapsed = 0;
        ...
        while (elapsed < duration)
        {
            elapsed = Mathf.MoveTowards(elapsed, duration, getTime());
	...
	public static IEnumerator ScaleTo(this Transform transform, Vector3 target, float duration, Easer ease, System.Func<float> getTime)
    {
        float elapsed = 0;
        var start = transform.localScale;
        var range = target - start;
	...
	public static float WaveUnscaled(float duration, float from, float to, float offsetPercent)
	{
		var range = (to - from) / 2;
		return from + range + Mathf.Sin(((Time.unscaledTime + duration * offsetPercent) / duration) * (Mathf.PI * 2)) * range;
	}
	...
	public static Color Wave(float duration, Color from, Color to)
    {
        return Color.Lerp(from, to, Wave(duration, 0, 1));
    }
	...
	public static Color MoveTowardsColor(this Color color, Color target, float maxDelta)
	{
	...
	public static IEnumerator Vector2(Vector2 start, Vector2 target, float duration, System.Action<Vector2> setter, Easer ease, System.Func<float> getTime)
    {
        float elapsed = 0;
        var range = target - start;
        while (elapsed < duration)
    ...
    public static IEnumerator Vector2(Vector2 start, Vector2 target, float duration, System.Action<Vector2> setter, Easer ease)
    {
        float elapsed = 0;
        var range = target - start;
        while (elapsed < duration)
    ...
    public static IEnumerator Vector2(Vector2 start, Vector2 target, float duration, System.Action<Vector2> setter)
    {
        return Vector2(start, target, duration, setter, Ease.Linear);
    }

    public static IEnumerator Vector2(Vector2 start, Vector2 target, float duration, System.Action<Vector2> setter, EaseType easeType)
    {
        return Vector2(start, target, duration, setter, Ease.FromType(easeType));
    }
	
	public static IEnumerator ColorLerp(Color start, Color target, float duration, System.Action<Color> setter)
	{
		float elapsed = 0;
		while (elapsed < duration)
		{
			elapsed += Time.deltaTime;
			setter(Color.Lerp(start, target, elapsed / duration));
			yield return null;	
		}
		setter(target);
	}
